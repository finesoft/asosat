...
public class SecurityContextParamReviser implements ParamReviser {
    ...
    public void accept(Object queryName, Object param) {
        if (param instanceof Map) {
            ...
            String currentUserId = SecurityContextHolder.currentUser().getId();
            if (currentUserId != null) {
                mapParam.putIfAbsent(Participator.CURRENT_USER_ID_KEY, currentUserId);
            }
            String currentOrgId = SecurityContextHolder.currentOrg().getId();
            if (currentOrgId != null) {
                mapParam.putIfAbsent(Participator.CURRENT_ORG_ID_KEY, currentOrgId);
            }
        }
    }
}
